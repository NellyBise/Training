...
export default function SelectedCard({ exercise, toggleExercise, index }) {
  return (
    <article className="relative border-[1px] p-2 border-black rounded-lg bg-white flex flex-col justify-center size-44">
      <h3 className="flex items-center justify-center h-full text-center text-sm font-bold">
        {exercise.name}
      </h3>
      <Image
      ...
      {toggleExercise ? (
        <button
          className="group absolute z-10 top-1.5 right-1.5 w-6 "
        ...
        >
          <svg
            ...
            viewBox="0 0 24 24"
            aria-hidden="true"
            className="bg-white/70 rounded-full duration-300 hover:bg-slate-200 cursor-pointer"
          >
            <path
              fill="#af1212"
